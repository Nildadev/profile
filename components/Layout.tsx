    ...
    <div className="min-h-screen flex flex-col">
      <header className="fixed top-0 left-0 right-0 z-50 flex justify-center py-8 pointer-events-none">
        <nav className={`
          ...
          flex items-center gap-2 px-3 py-2 rounded-full glass border border-white/5
          ${scrolled ? 'scale-90 opacity-95 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.3)]' : 'scale-100'}
        `}>
          <Link to="/" className="w-10 h-10 flex items-center justify-center bg-brand-primary rounded-full group mr-4 relative">
            <span className="text-white font-black text-sm group-hover:rotate-[360deg] transition-transform duration-700">N</span>
            {isSyncing && (
              <span className="absolute -top-1 -right-1 w-3 h-3 bg-white rounded-full flex items-center justify-center shadow-lg">
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  to={item.path}
                  className={`
                    px-6 py-2 rounded-full text-[11px] font-black uppercase tracking-[0.2em] transition-all duration-500
      ...
      <footer className="py-32 bg-black text-white px-8">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-20">
          <div className="col-span-2 space-y-8">
            <h2 className="text-4xl font-black tracking-tighter italic">Nildadev.</h2>
          ...
          <div className="space-y-6">
            <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-brand-primary">Navigation</h4>
          ...
          <div className="space-y-6">
            <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-brand-primary">Social</h4>
            <div className="flex flex-col gap-4 text-sm font-bold text-slate-400">
        ...
        <div className="max-w-7xl mx-auto mt-32 pt-12 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6">
          <p className="text-[9px] font-black uppercase tracking-[0.5em] text-slate-700">© 2024 DESIGNED BY NILDADEV</p>
